...
function activate(context) {
    ...
    let registration = vscode.workspace.registerTextDocumentContentProvider(jscpdSchema, provider);

    let disposableShowLines = vscode.commands.registerCommand('jscpd.showLines', showLines);

    let disposable = vscode.commands.registerCommand('extension.jscpd', function (explorer) {

        const currentPath = (explorer && path.dirname(explorer._fsPath)) || vscode.workspace.rootPath;
        ...
        vscode.window.showQuickPick(quickPickList).then(({data}) => {
            if (data === "REPORT") {
                ...
                var previewUri = vscode.Uri.parse(jscpdSchema + '://authority/jscpd?x=' + new Date().getTime().toString());
                vscode.commands.executeCommand('vscode.previewHtml', previewUri, vscode.ViewColumn.One, 'JSCPD').then(() => {
                }, (error) => {
